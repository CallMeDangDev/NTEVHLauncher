function initBottomBar() {
    ...
    document.getElementById('menuLinuxMode')?.addEventListener('click', () => {
        dropdown?.classList.remove('open');
        document.getElementById('btnMenu')?.classList.remove('active');
        S.cfg.linuxMode = !S.cfg.linuxMode;
        saveSettings();
        updateLinuxToggle();
        toast(S.cfg.linuxMode ? 'Đã bật chế độ Linux / Wine' : 'Đã tắt chế độ Linux / Wine', 'info');
    });
...
function startInstall() {
    ...
    prog.style.display = '';
    if (bridge()) {
        bridge().StartInstallation(S.gamePath, S.cfg.vhMode, S.cfg.backup, !!S.cfg.linuxMode);
    } else {
        simulateInstall();
    }
}
...
function updateLinuxToggle() {
    const t = document.getElementById('linuxModeToggle');
    if (t) t.classList.toggle('on', !!S.cfg.linuxMode);
}
