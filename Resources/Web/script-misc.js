...
async function loadSettings() {
    ...
    if (bridge()) {
        try {
            ...
            if (j) {
                ...
                Object.assign(S.cfg, parsed);
                S.gamePath = S.cfg.gamePath || '';
                if ('linuxMode' in parsed) firstRun = false;
            }
        } catch(e) {}
    } else {
        firstRun = false;
    ...
    if (S.gamePath && !S.installing) {
        setTimeout(() => { if (!S.installing) startInstall(); }, 800);
    } else {
        S.autoCheckDone = false;
    }
    if (typeof updateLinuxToggle === 'function') updateLinuxToggle();
}

function showOsChoiceOverlay() {
    const overlay = document.getElementById('osChoiceOverlay');
    if (!overlay) return;
    overlay.style.display = '';

    document.getElementById('oscWindows')?.addEventListener('click', () => {
        S.cfg.linuxMode = false;
        saveSettings();
        overlay.style.display = 'none';
        if (typeof updateLinuxToggle === 'function') updateLinuxToggle();
        S.autoCheckDone = false;
    ...
    document.getElementById('oscLinux')?.addEventListener('click', () => {
        S.cfg.linuxMode = true;
        saveSettings();
        overlay.style.display = 'none';
        if (typeof updateLinuxToggle === 'function') updateLinuxToggle();
        S.autoCheckDone = false;
...
async function browseFolder() {
    if (bridge()) {
        ...
        if (p === "?INVALID") {
            toast('Không tìm thấy thư mục chứa Neverness to Everness!', 'err');
            return false;
        }
        if (p) {
            S.cfg.gamePath = p;
...
function initAudioPlayer() {
    ...
    function setupAnalyser() {
        ...
        try {
            ...
            _audioCtx.createMediaElementSource(audio).connect(an);
            an.connect(_audioCtx.destination);
            window._musicAnalyser = an;
...
function fcSetCurrentFont(name) {
    ...
    if (name) {
        ...
        nameEl.textContent = 'Font gốc (Việt Hoá)';
...
async function fcRevert() {
    if (!S.gamePath) { toast('Chưa chọn thư mục game!', 'err'); return; }
    const confirmed = await showConfirm('Xóa font tuỳ chỉnh và dùng lại font gốc của bản Việt Hoá?');
    if (!confirmed) return;
